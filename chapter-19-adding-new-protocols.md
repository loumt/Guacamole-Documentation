...
在这一点上，如果你构建，安装，连接插件，你可以看见一个灰色的显示界面,这种练习是非常短暂的。然而，由于插件发送的唯一数据是在用户第一次加入时发送的。服务器在连接的剩余生命中没有发送的任何数据将导致客户机断开连接，假设连接已经停止响应。一旦加入弹跳球，它就会被纠正。

> 添加一个球

本教程是关于如何制作一个跳跃的球“客户”，所以很自然地我们需要一个球反弹。虽然我们可以在远程显示器上反复绘制和擦除一个球，但更有效的方法是利用Guacamole的层次。远程显示有一个根层,`GUAC_DEFAULT_LAYER,但也可以有无限多的其他子层，它们本身可以有子层，等等。每一层都可以在内部和相对于另一层动态重新定位。因为这些层的合成是由远程显示处理的，而且很可能是硬件加速的，这是一种更好的方式来反复地重新定位我们期望经常移动的东西。`

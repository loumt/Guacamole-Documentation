...
用户们通过他们的浏览器连接到guacamole服务器。这个guacamole客户端，是由JavaScript编写,在guacamole服务器中通过一个webSever向用户提供服务.一旦加载,这个客户端通过Guacamole协议回连至服务端.
...
开发了一种更快的基于文本的协议，可以呈现多个远程桌面协议的特性，而不仅仅是VNC。整个系统被重新设计成一个标准的守护进程，guacd和一个公共库，libguac，它驱动了守护进程和协议支持，从而变得可扩展。

项目的范围从一个足够的VNC客户机扩展到一个高性能的HTML5远程桌面通道和通用API。在其当前状态下，可以将Guacamole用作访问运行不同远程桌面服务器的任意数量机器的中心通道。它提供可扩展的身份验证，在这种情况下，您需要更专业的、基于html5的远程访问的通用API。
